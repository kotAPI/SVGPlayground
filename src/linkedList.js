var LinkedList = function(){
	...
	api.insertToList = function(val){
		...
		if(api.head===null){
			...
			message = {msg:"SPAWN_NODE",params:{idx:0,val:node.val,x:api.horizontalOffset,y:100}}
			...
			while(currentNode.next!==null){
				...
				message = {msg:"SPAWN_EDGE",params:{from:index-1,to:index,node1:{x:(index)*api.horizontalOffset,y:100},node2:{x:(index+1)*api.horizontalOffset,y:100}}}
				messages.push(message)
				api.messages.push(message)
			...
			message = {msg:"SPAWN_NODE",params:{idx:index,val:node.val,x:(index +1)*api.horizontalOffset,y:100}}
			messages.push(message)
			api.messages.push(message)
	...
	api.removeNode = function(val){
		var currentNode = api.head;
		if(api.head.next == null){return false}
