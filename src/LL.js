...
// // //lib.createNode(1, 10, 100, 100, 20, 2000).then(node => lib.moveNode(1, 50, 100, 500))


// // lib.createEdge(0, 1, 0, 0, 100, 100, 1000).then(()=> lib.moveEdge(0,1, 60, 10, 500, 500, 1000))
...
var LinkedList = function(){
	...
	api.addToList = function(value){
		...
		if(api.head ===null){
			api.head = newNode
			// create node
			lib.createNode(index,api.head.value,LEFTOFFSET*index + CANVASOFFSET,200,20,200)
			return;
		}
		...
		// draw edge
		// create node
		lib.createNode(index+1,newNode.value ,LEFTOFFSET*(index+1) +CANVASOFFSET,200,20,200).then(()=>{
			lib.createEdge(index,index+1, LEFTOFFSET*(index) +CANVASOFFSET,200,LEFTOFFSET*(index+1) +CANVASOFFSET,200,300)
		})
	...
	api.addNodeAt = function(index,value){
		...
		if(index === 0){
			lib.createNode(index,newNode.value ,LEFTOFFSET*(index) +CANVASOFFSET,300,20,200).then(()=>{
				lib.createEdge(index,index+1,LEFTOFFSET*(index) +CANVASOFFSET,300,LEFTOFFSET*(index) +CANVASOFFSET,200,300)
				.then(()=>{
					...
					for(let i=length-2; i>index;i--){
						lib.changeEdgeID("edge"+(i-1)+"-"+i,"edge"+i+"-"+(i+1))
						//lib.moveEdge(i-1,i,LEFTOFFSET*(i) +CANVASOFFSET,200,LEFTOFFSET*(i+1) +CANVASOFFSET,200,200)
					}
					for(let i=length-2; i>index;i--){
						lib.moveEdge(i,i+1,LEFTOFFSET*(i) +CANVASOFFSET,200,LEFTOFFSET*(i+1) +CANVASOFFSET,200,200)
					}
					...
					lib.moveEdge(index,index+1,LEFTOFFSET*(index) +CANVASOFFSET,200,LEFTOFFSET*(index+1) +CANVASOFFSET,200,200)
		...
		if(index>api.listLength()){
			console.log("pliz entr a vald numbr")
			return
		}
		...
		while(currentNode!=null){
			if(index === i){
				...
				lib.createNode(index,newNode.value ,LEFTOFFSET*(index) +CANVASOFFSET,300,20,300).then(()=>{
					lib.createEdge(index,index+1,LEFTOFFSET*(index) +CANVASOFFSET,300,LEFTOFFSET*(index) +CANVASOFFSET,200,300)
					.then(()=>{
						lib.moveEdge(index-1,index,LEFTOFFSET*(index-1) +CANVASOFFSET,200,LEFTOFFSET*(index) +CANVASOFFSET,300,200)
						.then(()=>{
							var length = api.listLength()
							...
							lib.moveEdge(index-1,index,LEFTOFFSET*(i-1) +CANVASOFFSET,200,LEFTOFFSET*(i) +CANVASOFFSET,200,200)
							lib.moveEdge(index,index+1,LEFTOFFSET*(i) +CANVASOFFSET,200,LEFTOFFSET*(i+1) +CANVASOFFSET,200,200)
